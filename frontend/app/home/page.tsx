...
const Page =async()=>{
  ...
  return (
    <div className="">
        ...
        <MainSection title={"In Threaters"} prop={data.InThreaters} />
          <MainSection title={"Trending Movies"} prop={data.Trending} />
          <MainSection title={"Netflix Originals"} prop={data.NetflixOriginals} isOriginal={true} />
...
const getData = async() => {
    const [ActionMovies,NetflixOriginals,ComedyMovies,Documentaries,RomanceMovies,Trending,TopRated,InThreaters] = await Promise.all([
      ...
      fetch(requests.fetchThreaterMovies).then((response) => response.json()).then((data) => data.results)
    ])
    return {
      ActionMovies,NetflixOriginals,ComedyMovies,Documentaries,RomanceMovies,Trending,TopRated,InThreaters
    }
}
export default Page
